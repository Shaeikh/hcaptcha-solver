...
class Manual_Classifier:
    def __init__(self, db_handler):
        ...
        self.captcha_selector = ttk.Combobox(
            self.selection_frame, textvariable=self.captcha_selection
        )
        ...
        self.captcha_label = tk.Label(
            self.selection_frame, text="Loading...", font=("Arial", 30)
        )
        ...
        self.menu_frame = tk.Frame(
            self.window, width=self.window.winfo_width(), height=10
        )
        self.go_back_button = tk.Button(
            self.menu_frame, text="Back", command=self.click_go_back, width=20, height=5
        )
        self.show_solved_button = tk.Button(
            self.menu_frame,
            text="Show Solved",
            command=self.show_all_solved,
            width=20,
            height=5,
        )  # TODO: new window with solved images
    ...
    def set_images(self):
        ...
        images_pil = [
            Image.open(IMAGES_DIR + image_path).resize((150, 150))
            for image_path in self.image_paths
        ]
        images_tk = [
            ImageTk.PhotoImage(image_pil, master=self.decision_frame)
    ...
    def click_continue(self):
        ...
        self.id_history = pd.concat(
            (
                self.id_history,
                pd.DataFrame({"id": self.image_paths, "action": self.selected_images}),
            ),
            ignore_index=True,
        )
        self.update_data()
    ...
    def click_go_back(self):
        if len(self.id_history) >= 9:
            ...
            self.db_handler.unlabel_images(
                self.id_history["id"].values[-9:].reshape((9, 1))
            )
    ...
    def set_info_label(self):
        solvedinfo = self.db_handler.get_solved_unsolved_for_captcha_string(
            self.captcha_selection.get()
        )
        self.info_label.config(
            text=f"Solved: {solvedinfo['solved']}, Unsolved: {solvedinfo['unsolved']}, Percentage: {round(solvedinfo['solved']/(solvedinfo['solved']+solvedinfo['unsolved'])*100,2)}%"
        )
    ...
    def show_all_solved(self):
        ...
        for i, image_path in enumerate(
            self.db_handler.get_labeled_true_images_paths(
                self.captcha_selection.get(), number=50
            )
        ):
            image_pil = Image.open(IMAGES_DIR + image_path).resize((60, 60))
        ...
        for i, image_path in enumerate(
            self.db_handler.get_labeled_false_images_paths(
                self.captcha_selection.get(), number=50
            )
        ):
            image_pil = Image.open(IMAGES_DIR + image_path).resize((60, 60))
